...
tokenizer_path = "../../wordpiece_tokenizer.json"
...
def __create_nsp_map(ab_row):
    ...
    first_pad_idx = x.index(pad_id) if num_pad > 0 else -1

    assert first_pad_idx > first_sep_idx or first_pad_idx == -1

    token_type_id = [ 0 if i <= first_sep_idx else 1 if i <= first_pad_idx else 2 for i in range(len(x)) ]  # [0,0,0,0,1,1,1,1,1,2,2,2]
...
def main():
    ...
    assert any([ "--block_size=" in each_argv for each_argv in sys.argv] ), "there should be --block_size arg!..."
